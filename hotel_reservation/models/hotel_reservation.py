...
class ResPartner(models.Model):
    ...
    id_type= fields.Many2one('idcard.master', 'ID Type')
    id_number= fields.Char(size=32, string='ID Number')
    sex= fields.Selection([('m','Male'),
                           ('f','Female'),],'Gender')
    dob= fields.Date('Date of Birth')
    nationality= fields.Many2one('res.country','Nationality')
    language= fields.Many2one('res.country','Language')
...
# --- adding new fields
    reseption = fields.Many2one('res.users','Reseption')
    bellboy =  fields.Many2one('hr.employee','Bellboy')
    guesttype =  fields.Many2one('guest.type','Guest Type')
    via =  fields.Selection([('direct','Direct'),
                             ('travel','Travel Agent'),],'Via',
                              default=lambda *a: 'direct')
    travel =  fields.Many2one('res.partner','Agent', domain="[('agent','=',True)]")
    source =  fields.Selection([('internal','Internal'),
                                ('gds','Through GDS'),],'Source',
                                 default=lambda *a: 'internal')
    gds_id =  fields.Char(size=24, string="GDS ID")
    pick_up =  fields.Boolean('Is Pickup Required')
    service_type = fields.Selection([('internal','Internal'),
    ...
    tranpsort_mode =  fields.Many2one('transport.mode','Transport Mode')
    pickup_location =  fields.Many2one('master.location','Pickup Location')
...
class IDCardMaster(models.Model):
    _name = 'idcard.master'
    _description = 'ID card Master'
    name = fields.Char(size=32, string='ID Type')
...
class GuestType(models.Model):
    ...
    name= fields.Char(size=32, string='Guest Type')
    code= fields.Char(size=4, string='Code')
    
class LocationMaster(models.Model): 
    _name = 'location.master'
    _description = 'Location Master'
    name= fields.Char(size=64, string='Location Name')
    code= fields.Char(size=4, string='Location Code')
    
class TransportPartner(models.Model):
    _name = 'transport.partner'
    _description = 'Transport Partner'
    name = fields.Char(size=32, string='Name')
    date_of_authorization= fields.Date('Authorization Date')
    partner_id=fields.Many2one('res.partner','Transporter Name')
    state= fields.Selection([('draft','Draft'),
                            ('confirmed','Confirmed')],'State')
    transport_info_ids = fields.One2many('transport.information','tran_info_id', string='Transport Type Information')
...
class TransportInformation(models.Model):
    ...
    name = fields.Many2one('transport.mode','Transport Mode')
    sale_price = fields.Float('Sale Price')
    to_location = fields.Many2one('location.master','Destination')
    tran_info_id = fields.Many2one('transport.partner','Transport Information ID')
    
class TransportMode(models.Model):
    _name = 'transport.mode'
    _description = 'Transport Mode'
    name = fields.Char(size=32,string="Transport Mode")
